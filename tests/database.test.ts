...
test('실제 migration의 제약과 A/B 사용자 RLS를 PostgreSQL 엔진에서 검증한다', async function () {
  ...
  try {
    ...
    const connection = await db.query<{ id: string }>(`insert into public.calendar_connections(user_id,provider,access_token_encrypted) values ('${a}','google','encrypted') returning id`);
    await db.query(`insert into public.external_events(user_id,connection_id,provider,external_id,title,starts_at,ends_at) values ($1,$2,'google','event-1','외부 일정',now(),now()+interval '1 hour')`, [a, connection.rows[0].id]);
    ...
    for (const table of [...Object.keys(fixtures), 'routine_logs', 'dashboard_settings', 'calendar_connections', 'external_events']) {
    ...
    for (const table of [...Object.keys(fixtures), 'routine_logs', 'dashboard_settings', 'calendar_connections', 'external_events']) await assert.rejects(db.query(`select * from public.${table}`), table + ' anon blocked');
    await user('a');
